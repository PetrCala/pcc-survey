...
test_that("pcc_sum_stats calculates correctly with some missing sample sizes", {
  ...
  df <- data.frame(
    sample_size = c(50, NA_real_, 100, 200, NA_real_),
  ...
  expect_equal(result$avg_n, mean(c(50, 100, 200), na.rm = TRUE))

  # median_n should be median of non-NA values
  expect_equal(result$median_n, stats::median(c(50, 100, 200), na.rm = TRUE))

  # ss_lt statistics should be calculated correctly
  expect_equal(result$ss_lt_50, 1 / 5) # 1 value < 50 out of 5 total
  expect_equal(result$ss_lt_100, 1 / 5) # 1 value < 100 out of 5 total
  expect_equal(result$ss_lt_200, 2 / 5) # 2 values < 200 out of 5 total
})
...
test_that("pcc_sum_stats calculates correctly with no missing sample sizes", {
  ...
  expect_equal(result$avg_n, mean(c(50, 100, 150, 200, 250)))

  # median_n should be median of all values
  expect_equal(result$median_n, stats::median(c(50, 100, 150, 200, 250)))

  # quantiles should be calculated correctly
  quantiles <- stats::quantile(c(50, 100, 150, 200, 250), probs = c(0.25, 0.75))
  expect_equal(result$quantile_1_n, as.numeric(quantiles[1]))
  expect_equal(result$quantile_3_n, as.numeric(quantiles[2]))

  # ss_lt statistics should be calculated correctly
  expect_equal(result$ss_lt_50, 1 / 5) # 1 value < 50
  expect_equal(result$ss_lt_100, 1 / 5) # 1 value < 100
  expect_equal(result$ss_lt_200, 3 / 5) # 3 values < 200
  expect_equal(result$ss_lt_400, 5 / 5) # 5 values < 400
})
